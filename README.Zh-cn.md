...
基于Gin + Vue + Element UI OR Arco Design OR Ant Design的前后端分离权限管理系统,系统初始化极度简单，只需要配置文件中，修改數据库连接，系统支持多指令操作，迁移指令可以让初始化數据库訊息变得更简单，服务指令可以很简单的启动api服务
...
2. 部门管理：配置系统组织机构（公司、部门、小组），树结构展现支持數据权限。
3. 岗位管理：配置系统用户所属担任职务。
4. 菜单管理：配置系统菜单，操作权限，按钮权限标识，接口权限等。
5. 角色管理：角色菜单权限分配、设置角色按机构进行數据范围权限划分。
6. 字典管理：对系统中经常使用的一些较为固定的數据进行维护。
7. 参數管理：对系统动态配置常用参數。
8. 操作日志：系统正常操作日志记录和查詢；系统异常訊息日志记录和查詢。
9. 登录日志：系统登录日志记录查詢包含登录异常。
1. 接口文档：根据业务代碼自动生成相关的api接口文档。
1. 代碼生成：根据數据表结构生成对应的增删改查相对应业务，全程可视化操作，让基本业务可以零代碼实现。
1. 表单构建：自定义頁面样式，拖拉拽实现頁面布局。
1. 服务监控：查看一些服务器的基本訊息。
1. 内容管理：demo功能，下设分类管理、内容管理。可以参考使用方便快速入门。
1. 定时任务：自动化任务，目前支持接口调用和函數调用。
...
[如何配置菜单訊息以及接口訊息](https://www.bilibili.com/video/BV1zv411B7nG) [必看]

[go-admin权限配置使用说明](https://www.bilibili.com/video/BV1rt4y197d3) [必看]

[go-admin數据权限使用说明](https://www.bilibili.com/video/BV1LK4y1s71e) [必看]
...
在项目启动时，使用`-a true` 系统会自动新增缺少的接口數据
```bash
