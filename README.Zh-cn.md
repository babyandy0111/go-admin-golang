...
[前端項目](https://github.com/go-admin-team/go-admin-ui)
...
2. 部門管理：配置系統组织机构（公司、部門、小组），樹結構展现支持資料權限。
...
1. API文档：根據业務代碼自动生成相关的apiAPI文档。
1. 代碼生成：根據資料表結構生成對應的增删改查相對應业務，全程可视化操作，让基本业務可以零代碼实现。
1. 表單建立：自定義页面主題，拖拉拽实现页面布局。
1. 服務監控：查看一些服務器的基本資訊。
1. 内容管理：demo功能，下设分類管理、内容管理。可以参考使用方便快速入门。
...
同时配套了系列教程包含视频和文档，如何从下载完成到熟练使用，强烈建议大家先看完这些教程再来实践本項目！！！

### 轻松实现go-admin写出第一个应用 - 文档教程

[步骤一 - 基礎内容介绍](https://doc.zhangwj.com/guide/intro/tutorial01.html)

[步骤二 - 实际应用 - 编写增删改查](https://doc.zhangwj.com/guide/intro/tutorial02.html)

### 手把手教你从入门到放弃 - 视频教程

[如何啟動go-admin](https://www.bilibili.com/video/BV1z5411x7JG)
...
在項目啟動时，使用`-a true` 系統会自动添加缺少的API資料
```bash
...
# 啟動容器，第一个go-admin是容器名字，第二个go-admin是镜像名稱
# -v 映射配置文件 本地路径：容器路径
...
# 建议不要直接使用 cnpm 安装依赖，会有各种诡异的 bug。可以透過如下操作解决 npm 下载速度慢的问题
...
`go-admin` 項目一直以来都是在 JetBrains 公司旗下的 GoLand 集成开发环境中进行开发，基于 **free JetBrains Open Source license(s)** 正版免费授權，在此表达我的谢意。
...
> 如果你觉得这个項目帮助到了你，你可以帮作者买一杯果汁表示鼓励 :tropical_drink:
