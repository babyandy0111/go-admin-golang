...
func (e Gen) GenCode(c *gin.Context) {
	...
	if err != nil {
		log.Error(err)
		e.Error(500, err, fmt.Sprintf("tableId參數接收失敗！错误详情：%s", err.Error()))
		return
	}
...
func (e Gen) GenApiToFile(c *gin.Context) {
	...
	if err != nil {
		log.Error(err)
		e.Error(500, err, fmt.Sprintf("tableId參數取得失敗！错误详情：%s", err.Error()))
		return
	}
...
func (e Gen) NOActionsGen(c *gin.Context, tab tools.SysTables) {
	...
	if err != nil {
		log.Error(err)
		e.Error(500, err, fmt.Sprintf("views目录建立失敗！错误详情：%s", err.Error()))
		return
	}
...
func (e Gen) genApiToFile(c *gin.Context, tab tools.SysTables) {
	...
	if err != nil {
		e.Logger.Error(err)
		e.Error(500, err, fmt.Sprintf("資料迁移模版解析失敗！错误详情：%s", err.Error()))
		return
	}
...
func (e Gen) GenMenuAndApi(c *gin.Context) {
	...
	if err != nil {
		e.Logger.Error(err)
		e.Error(500, err, fmt.Sprintf("tableId參數解析失敗！错误详情：%s", err.Error()))
		return
	}
