...
func (e SysPost) Get(c *gin.Context) {
	...
	if err != nil {
		e.Error(500, err, fmt.Sprintf("岗位訊息獲取失敗！错误详情：%s", err.Error()))
		return
	}
...
func (e SysPost) Insert(c *gin.Context) {
	...
	err = s.Insert(&req)
	if err != nil {
		e.Error(500, err, fmt.Sprintf("新建岗位失敗！错误详情：%s", err.Error()))
		return
	}
	e.OK(req.GetId(), "創建成功")
}
...
func (e SysPost) Update(c *gin.Context) {
	...
	err = s.Update(&req)
	if err != nil {
		e.Error(500, err, fmt.Sprintf("岗位更新失敗！错误详情：%s", err.Error()))
		return
	}
	e.OK(req.GetId(), "更新成功")
}
...
// @Description 删除數据
// @Tags 岗位
// @Param id body dto.SysPostDeleteReq true "请求参數"
// @Success 200 {object} response.Response "{"code": 200, "data": [...]}"
...
func (e SysPost) Delete(c *gin.Context) {
	...
	err = s.Remove(&req)
	if err != nil {
		e.Error(500, err, fmt.Sprintf("岗位删除失敗！错误详情：%s", err.Error()))
		return
	}
	e.OK(req.GetId(), "删除成功")
}
