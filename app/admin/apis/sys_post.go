...
func (e SysPost) Get(c *gin.Context) {
	...
	if err != nil {
		e.Error(500, err, fmt.Sprintf("職稱訊息取得失敗！錯誤詳情：%s", err.Error()))
		return
	}
...
func (e SysPost) Insert(c *gin.Context) {
	...
	err = s.Insert(&req)
	if err != nil {
		e.Error(500, err, fmt.Sprintf("新建職稱失敗！錯誤詳情：%s", err.Error()))
		return
	}
	e.OK(req.GetId(), "建立成功")
}
...
func (e SysPost) Update(c *gin.Context) {
	...
	err = s.Update(&req)
	if err != nil {
		e.Error(500, err, fmt.Sprintf("職稱更新失敗！錯誤詳情：%s", err.Error()))
		return
	}
	e.OK(req.GetId(), "更新成功")
}
...
func (e SysPost) Delete(c *gin.Context) {
	...
	err = s.Remove(&req)
	if err != nil {
		e.Error(500, err, fmt.Sprintf("職稱刪除失敗！錯誤詳情：%s", err.Error()))
		return
	}
	e.OK(req.GetId(), "刪除成功")
}
